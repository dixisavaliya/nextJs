...
const props = {
	...
	colours: {
		// Cache the colours
		globeDots: "rgb(230, 230, 230)", // No need to use the Three constructor as this value is used for the HTML canvas drawing 'fillStyle' property
		lines: new THREE.Color('#005CB9'),
		lineDots: new THREE.Color('#005CB9'),
	},
	alphas: {
...
const animateDots = () => {
	...
	for (let i = 0; i < groups.lineDots.children.length; i++) {
		...
		if (dot._path === null) {
			...
			if (dot._path != null && dot._pathIndex > 0) {
				// Show the dot
				if (!dot.visible) {
					dot.visible = true;
				}
...
const animateCountryCycle = () => {
  ...
  const { countries } = animations;
	...
	if (countries.current <= countries.total) {
		...
		var e = (i = countries.current / countries.total) < 0.5 ? 2 * i * i : (4 - 2 * i) * i - 1,
			t = (camera.angles.current.azimuthal - camera.angles.target.azimuthal) * e;
		
		t = camera.angles.current.azimuthal - t;
		camera.controls.setAzimuthalAngle(t);
		var n = (camera.angles.current.polar - camera.angles.target.polar) * e;

		n = camera.angles.current.polar - n;
		camera.controls.setPolarAngle(n);
		countries.current++;
		/* const { current, target } = camera.angles;
